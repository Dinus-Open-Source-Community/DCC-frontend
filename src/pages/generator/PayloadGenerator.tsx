...
export default function PayloadGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Gagal compile payload.";
      setError(message);
      setStatus("error");
    }
  };
    ...
    <div className="flex flex-col px-14 py-10 w-full h-[calc(100vh-1px)] overflow-y-auto custom-scrollbar">
      ...
      <div className="border border-green/30 rounded-[10px] p-8 max-w-full bg-black">
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-green text-sm mb-2">
              Server IP / Domain
            </label>
            <input
              type="text"
          ...
          <div>
            ...
            <div className="relative">
              ...
              <div className="absolute right-4 top-0 bottom-0 flex flex-col items-center justify-center gap-[3px]">
                <button
                  type="button"
                  onClick={() => !isProcessing && setPort((p) => p + 1)}
                  disabled={isProcessing}
                  className="text-white hover:text-green transition-colors focus:outline-none flex justify-center disabled:opacity-50"
                >
                  <svg
                    width="8"
                    height="4"
                ...
                <button
                  type="button"
                  onClick={() =>
                    !isProcessing && setPort((p) => Math.max(0, p - 1))
                  }
                  disabled={isProcessing}
                  className="text-white hover:text-green transition-colors focus:outline-none flex justify-center disabled:opacity-50"
                >
          ...
          <div className="bg-[#050A07] border border-green/20 rounded-lg p-6 space-y-4 mt-6">
            <label className="flex items-center gap-3 cursor-pointer group">
              <div className="relative flex items-center justify-center">
                ...
                <Check
                  size={12}
                  className="absolute text-green opacity-0 peer-checked:opacity-100 pointer-events-none"
                  strokeWidth={3}
                />
              </div>
            ...
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-green text-sm">
                Enable Suicide Mode (hanya bekerja jika Anti-Debug atau Anti-VM
                aktif)
              </span>
            </label>
                ...
                <div className="mt-4 w-full bg-black border border-orange-500 rounded-lg p-6 space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-orange-500 text-sm font-medium">
                      Compiling payload di server… (30-60 detik, jangan tutup
                      halaman)
                    </p>
                  </div>
              ...
              {status === "success" && (
                <div className="mt-4 w-full bg-black border border-green rounded-lg p-6 flex justify-center items-center gap-3">
                  ...
                  <p className="text-green text-sm font-medium">
                    Generation complete! Download started otomatis. Cek folder
                    Downloads kamu.
                  </p>
                </div>
              )}
              ...
              {status === "error" && error && (
                <div className="mt-4 w-full bg-black border border-red-500 rounded-lg p-4 flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-red-400 text-xs font-mono mt-1 whitespace-pre-wrap break-words">
                      {error}
                    </p>
                  </div>
                </div>
