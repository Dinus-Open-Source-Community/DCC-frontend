    ...
    <span className="inline-flex items-center gap-2">
      <span
        className={cn(
          ...
          online
            ? "bg-green animate-pulse shadow-[0_0_8px_rgba(0,255,65,0.7)]"
            : "bg-red-500/70",
        )}
        aria-hidden
      />
      <span
        className={cn(
          "text-sm font-normal",
          online ? "text-green" : "text-red-400/80",
        )}
      >
        {status}
      </span>
...
export default function InfectedAgentsTable({
  rows,
}: InfectedAgentsTableProps) {
  ...
  return (
    <div className="col-span-1 lg:col-span-2 rounded-md border border-green/30 bg-[#0A0F0A] px-4 py-3 sm:px-5 sm:py-4 text-green flex flex-col">
      <div className="flex items-center justify-between text-xs uppercase tracking-wider font-medium mb-3 shrink-0">
        ...
        <span className="text-xs text-green/60 font-mono normal-case tracking-normal">
        ...
        <table className="w-full text-left border-collapse whitespace-nowrap">
          <thead className="sticky top-0 z-10">
            <tr className="border-b border-green/30">
              <th className="pt-3 pb-2 pr-4 text-start bg-[#0A0F0A]">
                Agent ID
              </th>
              <th className="pt-3 pb-2 pr-4 text-start bg-[#0A0F0A]">
                IP Address
